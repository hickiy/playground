  ...
  <transition name="viewer-fade">
    <div tabindex="-1" ref="el-image-viewer__wrapper" class="el-image-viewer__wrapper" :style="{ 'z-index': viewerZIndex }">
      ...
      <div class="el-image-viewer__btn el-image-viewer__actions">
        <div class="pointer el-image-viewer__actions__inner">
          ...
          <i class="el-icon-files" @click="handleActions('downloadAll')"></i>
          <i class="el-icon-download" @click="handleActions('downloadCurrent')"></i>
        </div>
      </div>
      <!-- CANVAS -->
    ...
    handleActions(action, options = {}) {
      ...
      switch (action) {
        ...
        case 'downloadCurrent':
          this.$download(this.currentImg, { fileName: this.currentImg.split('/').pop() });
          break;
        case 'downloadAll':
          if (this.urlList.length == 1) {
            this.$download(this.currentImg, { fileName: this.currentImg.split('/').pop() });
          } else {
            this.$confirm('确定要下载所有图片吗？', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              this.$batchDownload(this.urlList);
            });
          }
          break;
      }
